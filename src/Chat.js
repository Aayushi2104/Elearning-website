  ...
  return (
    <div className="flex flex-col h-full p-4 bg-gray-900 text-white">
      ...
      <div className="flex">
        ...
        <button
          onClick={handleSend}
          className="px-4 py-2 bg-teal-500 text-white rounded-r-lg hover:bg-teal-600"
        >
          Send
        </button>
      </div>
...
const MessageBubble = ({ text, sender }) => {
  ...
  return (
    <animated.div style={animationProps} className={`mb-2 ${sender === "You" ? "text-right" : "text-left"}`}>
      <span className={`inline-block px-4 py-2 rounded-lg ${sender === "You" ? "bg-teal-500" : "bg-gray-700"}`}>
        {text}
      </span>
    </animated.div>
  );
    ...
    <div className="text-left mb-2">
      <div className="inline-block px-4 py-2 bg-gray-700 rounded-lg flex space-x-1">
        {typingDots.map((dot, index) => (
          <span key={index} className={`w-2 h-2 bg-teal-400 rounded-full animate-bounce ${dot}`} />
        ))}
      </div>
    </div>
  );
